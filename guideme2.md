...
**Caveat:** that 0-findings result is from Trivy locally, not yet from a real CodeBuild run against this account's actual Inspector v2 scan (no `ecr`/`codebuild` stack was up to test against at the time — the two scanners have overlapping but not identical CVE databases). Treat it as strong evidence, not a guarantee, until the next full redeploy's pipeline run confirms `CRITICAL=0` for real via the scan gate itself.
...
**Immediate next step (as of 2026-08-15):** the ECR-push fix, the scan-gate poll-loop fix, and the Dockerfile hardening (npm/npx/corepack removal + `apk upgrade`) are all committed. The full stack (all 12 stacks) is currently torn down — nothing is deployed. Next real test: redeploy from step 3, and this time watch specifically whether the Build stage's scan-gate step reports `CRITICAL=0` for real (see this section's Symptom 3 caveat — that's confirmed locally via Trivy, not yet via this account's actual Inspector v2 scan). If it does, this should be the first fully green `Source → Build → Approval → Deploy` run this project has had.
